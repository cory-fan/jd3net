...
class IsotropicAttn(nn.Module):

    def bayer_to_rgb(self,img):
        cfa_pattern = img[:,1:,:,:]
        rgb = cfa_pattern*img[:,0:1,:,:]
        return torch.concat((rgb,cfa_pattern),dim=1)

    def __init__(self,img_channel=6,downsample=4,width=172,depth=84,drop_path_rate=0,start_kernel_size=3):
        super().__init__()
        self.downsample = downsample
        self.start_conv = nn.Conv2d(img_channel,width,start_kernel_size,padding="same")
        self.down_conv = nn.Conv2d(width,width,downsample,stride=downsample)
        self.blocks = nn.ModuleList()
...
class MHIsotropicAttn(nn.Module):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.in_channels=12
        self.out_channels=3
        self.network = IsotropicAttn(*args,**kwargs)
    ...
    def forward(self,x):
        ...
        x = self.network(x)
        return x[0:B],x[B:2*B],x[2*B:3*B]
