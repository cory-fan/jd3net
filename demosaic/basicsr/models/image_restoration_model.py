...
class ImageRestorationModel(BaseModel):
    ...
    def feed_data(self, data, is_val=False):
        ...
        #print(self.lq.flatten(0,2).min(dim=-1)[0],data['i'],end="\r")
        if 'gt' in data:
    ...
    def dist_validation(self, dataloader, current_iter, tb_logger, save_img, rgb2bgr, use_image):
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            img_name = osp.splitext(osp.basename(val_data['lq_path'][0]))[0] if 'lq_path' in val_data.keys() else "Image %i"%idx
            ...
            sr_img = tensor2img([visuals['result']], rgb2bgr=rgb2bgr, out_type=None)
            if 'gt' in visuals:
                gt_img = tensor2img([visuals['gt']], rgb2bgr=rgb2bgr, out_type=None)
                del self.gt
                ...
                if sr_img.shape[2] == 6:
                    ...
                    if self.opt['is_train']:
                        ...
                        save_gt_img_path = osp.join(
                            self.opt['path']['visualization'], dataset_name,
                            f'{img_name}_gt.png')
                    imwrite(sr_img[:,:,:3]*255, save_img_path)
                    imwrite(gt_img[:,:,:3]*255, save_gt_img_path)
