...
class IsotropicPosNet(nn.Module):
    ...
    def generate_qxq_bayer(self,H,W,Q):
        ...
        return torch.tensor(filter).cuda().float()
    ...
    def __init__(self,img_channel=3,downsample=3,width=64,depth=64,drop_path_rate=0,start_kernel_size=3,cfa_size=1):
        ...
        self.start_conv = nn.Conv2d(img_channel+3,local_channel,start_kernel_size,padding="same")
    ...
    def forward(self,x):
        ...
        if (C==3):
            if (self.filter==None or self.filter.shape!=(x.shape[2],x.shape[3])):
                self.filter = self.generate_qxq_bayer(x.shape[2],x.shape[3],self.cfa).permute(2,0,1)
            x = torch.concat((x,self.filter.view(1,3,x.shape[2],x.shape[3]).repeat(B,1,1,1)),dim=1)
